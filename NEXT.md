...
- [x] Aura类型：持续影响范围内敌人或玩家 — 剧毒光环(poison_aura): 90px范围/0.5秒周期毒伤, 脉冲光环绘制
- [x] Summon类型：生成自动作战单位 — 暗影小鬼(shadow_imp): 3秒冷却召唤1个, 8秒寿命, 自动追击+接触伤害
...
- [x] Consume（吞噬武器 — 预留接口，注释标注为未来功能）— consumeWeapon() 方法 + 攻击触发点注释
...
### 5.3 替换占位素材
- [ ] 玩家：当前仍使用 `player/hero.png` 主贴图；hero_idle/hero_move 文件是占位小图，正式替换前不要接入运行时
- [ ] 敌人：当前仍使用配置中的主贴图；toxic_slime/demon_bat/bandit_hooded idle 文件是占位小图，正式替换前不要接入运行时
- [ ] Boss：当前仍使用 `bosses/dark_knight_flame.png` 主贴图；idle/attack 文件是占位小图，正式替换前不要接入运行时
...
- [x] 经验物对象池
- [x] 伤害数字对象池
- [x] 空间分区（网格或四叉树）替代 O(n²) 碰撞 — SpatialGrid 128px 均匀网格，每帧重建，6处碰撞循环替换为 query()
- [x] 屏幕外实体跳过更新（isOnScreen 判定，敌人/弹丸/粒子跳过更新与渲染，Boss 始终更新，分离逻辑只检查屏内敌人）
- [x] 目标：100+敌人稳定运行 — 空间分区+对象池+屏幕外剔除三重优化
...
针对玩家实机反馈发现的体验问题，补充阶段1~6 验收盲点。

### 7.1 BOSS掉落拾取流程
原设计 Boss 死亡立即进入胜利剧情，导致玩家无法拾取 Boss 掉落物（经验宝石、宝箱）。
- [x] Boss 死亡后进入 8 秒拾取宽限期（bossDefeatedGraceTimer），玩家可自由移动拾取
- [x] 宽限期内仍更新玩家、拾取物、粒子、摄像机，保持操作手感
- [x] 宽限期结束才触发胜利剧情与关卡切换（onBossDefeated 拆分为宽限期 + startVictorySequence）
- [x] 防重复触发保护（onBossDefeated 首行 return 守卫）

### 7.2 关卡差异化
原设计两关共用固定随机种子 42 与大量重叠地面素材，导致第二关地图与第一关视觉几乎一致。
- [x] 随机种子改为基于关卡主题生成（village/mine 产生不同地形布局）
- [x] 关卡基础底色区分（village 泥土色 #2a2218 / mine 深紫黑 #1a1520）
- [x] 地面素材池去重（village 以草地泥土为主 / mine 以苔藓石碎石为主）
- [x] village 关补充残垣断壁边界墙（wallTiles），与 mine 的石砌墙区分
- [x] 暗斑路径采用主题色调（mine 更暗的紫黑）

### 7.3 地图视觉平滑
原 tile 渲染每格独立绘制，格子边缘明显，过渡生硬。
- [x] 相邻 tile 间绘制半透明渐变过渡带（横向 + 纵向），弱化格子边界
