...
assert.equal(Object.keys(config.WEAPONS).length, 14, 'expected 14 weapons (11 base + 3 evolved)');
...
console.log(`Smoke checks passed: ${resources.size} configured resources, 14 weapons (11 base + 3 evolved), 20 upgrades, 23 enemies, 2 levels with phased spawning, portrait layout, pickup attraction, prerequisite system, settings overlay, story UI separation, off-screen culling, BGM tracks, object pools, mimic state machine, expanded asset manifest (${assetCount} assets), weapon evolution system, spatial grid collision.`);
